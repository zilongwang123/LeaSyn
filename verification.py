...
def selfCompositionObservationEquivalence(wireId, obsDict, prefix):
    ...
    for obsId in obsDict.keys():
        # if obsId == "DIVISION":
        if obsId.endswith("_PI"):
            obsDict_1[obsId] =obsDict[obsId]
        else:
            obsDict_2[obsId] =obsDict[obsId]
    if len(obsDict.keys()) > 0:
        if wireId == "src_equiv":
            if len(obsDict_1) == 0:
                # filtered semantic
                condition+="\twire {} = ( ! ( Retire_obs_trg_arg0_trg_right && Retire_obs_trg_arg0_trg_left ) ) || ( {} ) ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict_2.keys() ] ))
            elif len(obsDict_2) == 0:
                # filtered semantic
                condition+="\twire {} =  ( {} ) ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict_1.keys() ] ))
            else:
                # filtered semantic
                condition+="\twire {}_1 = ( ! ( Retire_obs_trg_arg0_trg_right && Retire_obs_trg_arg0_trg_left ) ) || ( {} ) ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict_2.keys() ] ))
                # # non-filtered semantic
                condition+="\twire {}_2 =  ( {} ) ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict_1.keys() ] ))
                condition+="\twire {} = {}_1 && {}_2;\n".format(wireId,wireId,wireId)
        else:
            condition+="\twire {} = {} ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict.keys() ] ))
    # if len(obsDict.keys()) > 0:
    #     condition+="\twire {} = {} ;\n".format(wireId, " && ".join( ["{}_{}".format(obsId, prefix) for obsId in obsDict.keys() ] ))
...
def selfCompositionCycleDelayedCheck(clock, bound, delay, cstrType):
    ...
    if CONF.propertyEncoding == "static":
        ...
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(bound) + 1) ) + 1} : 0 ] bound = {bound};\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(bound) + 1) ) + 1} : 0 ] counter = {bound} - {delay};\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(delay) + 1) ) + 1} : 0 ] delay = {delay} + 1;\n"
        verificationConditions += "\talways @ (posedge {}) begin\n".format(clock)
        verificationConditions += f"\t\tif (counter > 0) begin\n"
        verificationConditions += f"\t\t\tcounter <= counter - 1;\n"
        verificationConditions += "\t\tend\n"
        ...
        verificationConditions += f"\t\t\tbound <= bound - 1;\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += f"\t\tif (delay > 0) begin\n"
        verificationConditions += f"\t\t\tdelay <= delay - 1;\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += "\tend\n"
        verificationConditions += "\t// update the states for verification\n"
        verificationConditions += f"\treg [{bound} - {delay} -1 : 0] state_trg_equiv = {math.pow(2,int(bound) - int(delay)) -1};\n"
        verificationConditions += "\talways @ (posedge {}) begin\n".format(clock)
        verificationConditions += f"\t\tif (counter > 0) begin\n"
        verificationConditions += f"\t\t\tstate_trg_equiv[0] <= state_trg_equiv[0] && trg_equiv;\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += "\tend\n"
        verificationConditions += f"\tgenvar i;\n"
        verificationConditions += f"\tgenerate\n"
        verificationConditions += f"\n\tfor (i = 1; i < {bound} - {delay}; i = i + 1) begin\n"
        verificationConditions += "\talways @ (posedge {}) begin\n".format(clock)
        verificationConditions += f"\t\tif (counter > 0) begin\n"
        verificationConditions += f"\t\t\tstate_trg_equiv[i] <= state_trg_equiv[i-1];\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += "\tend\n"   
        verificationConditions += f"\tend\n"
        verificationConditions += f"\tendgenerate\n"
        verificationConditions += "\twire fin_state_trg_equiv = ( delay > 0 ) ||  state_trg_equiv[ bound-counter] ;\n"
    ...
    elif CONF.propertyEncoding == "dynamic":
        instr_counter = CONF.instrDelayed 
        verificationConditions = ""
        verificationConditions += "\t// auxiliary register for Bound and Counter\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(bound) + 1) ) + 1} : 0 ] bound = {bound};\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(bound) + 1) ) + 1} : 0 ] counter = {bound} - {delay};\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(delay) + 1) ) + 1} : 0 ] delay = {delay} + 1;\n"
        verificationConditions += f"\treg  [{ math.floor( math.log2(int(instr_counter) + 1) ) + 1} : 0 ] instr_counter = {instr_counter} ;\n"
        verificationConditions += f"\treg  state_trg_equiv = 1 ;\n"
        verificationConditions += f"\treg  delayed_state_trg_equiv = 1 ;\n"
        ...
        verificationConditions += f"\t\tstate_trg_equiv <= state_trg_equiv && trg_equiv;\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += f"\t\tdelayed_state_trg_equiv <= state_trg_equiv ;\n"
        verificationConditions += f"\t\tif ( state_trg_equiv == 0 && Retire_obs_trg_arg0_trg_left && Retire_obs_trg_arg0_trg_right && instr_counter > 0 ) begin\n"
        verificationConditions += f"\t\tinstr_counter <= instr_counter - 1;\n"
        verificationConditions += "\t\tend\n"
        verificationConditions += "\tend\n"

        verificationConditions += "\twire delayed_src_equiv = ( instr_counter == 0 ) || src_equiv ;\n"
        verificationConditions += "\twire fin_state_trg_equiv = ( instr_counter != 0 ) ||  delayed_state_trg_equiv ;\n"
        

        verificationConditions += selfCompositionAssume("init_state_equiv")
        verificationConditions += selfCompositionAssume("delayed_src_equiv")
        verificationConditions += selfCompositionAssume("state_invariant")
        verificationConditions += selfCompositionAssert("fin_state_trg_equiv")
    else:
        logfile("The property encoding is not well defined!")
...
def constructProductCircuit(outFolder, srcObsVar, srcCandObsVar, trgObsVar, state, invVars, clock, filtertype):
    ...
    if WIRE_DECLARATION_PLACEHOLDER in productCircuit:
        ...
        wireDeclaration += selfCompositionVariableDecl(srcCandObsVar,  "src_cand")
    ...
    # 3.  stuttering signal
    if STUTTERING_SIGNAL_PLACEHOLDER in productCircuit:
        stutteringSignal = ""
        ...
        productCircuit = productCircuit.replace(STUTTERING_SIGNAL_PLACEHOLDER, stutteringSignal)
    ...
    if MODULE_DECLARATION_PLACEHOLDER in productCircuit:
        ...
        for obsId in srcCandObsVar:
            for obs in srcCandObsVar[obsId]:
                leftVarMap[obs["var"]] = "{}_src_cand_left".format(obs["var"])
                rightVarMap[obs["var"]] = "{}_src_cand_right".format(obs["var"])
    ...
    # 6. State invariants constraints
    ...
    if VERIFICATION_CONDITIONS_PLACEHOLDER in productCircuit:
        ...
        verificationConditions += "\n"
        # 7.x candidate contract atoms
        verificationConditions += "\t// candidate contract atoms\n"
        verificationConditions += selfCompositionObservationEquivalence("src_cand_equiv", srcCandObsVar, "src_cand")
        verificationConditions += "\n"
        ...
        verificationConditions += "\t// property checked\n"
        verificationConditions += selfCompositionCycleDelayedCheck(clock, CONF.BMCBound, CONF.cycleDelayed, filtertype)
        verificationConditions += "\n"
...
def verify(trgObservations, cstrtype, filtertype):
    ...
    outFolder = CONF.outFolder + "/temp" + "/" + filtertype + "_" + cstrtype
...
def boundedVerify(srcObservations, trgObservations, candidateContractAtoms, stateInvariant, auxVars, metaVars, filtertype):
    state = CONF.state
    module = CONF.module
    outFolder = CONF.outFolder + "/temp"
    
    trg_obs_dict, auxVars_dict = initObservations(trgObservations, {}, {}, "obs_trg")
    src_cand_obs_dict, auxVars_dict2 = initObservations(candidateContractAtoms, {}, {}, "obs_src_cand")
    # construct yosys script
    yosysScript = ""
    ...
    ## 3.1. inline source observations
    log("Inline src observations")
    srcObsVar, script = inlineObservations(outFolder, metaVars, auxVars, srcObservations, module, "obs_src")
    yosysScript += script

    ## 3.2. inline candidate contract atoms
    log("inline candidate contract atoms")
    srcCandObsVar, script = inlineObservations(outFolder, metaVars, auxVars, candidateContractAtoms, module, "obs_candidate")
    yosysScript += script
    ...
    ## 6. Create product circuit
    log("Create product circuit")
    constructProductCircuit(outFolder, srcObsVar, srcCandObsVar, trgObsVar, trgStateVars, srcInvsVars, CONF.clockInput, filtertype)
    time25 = datetime.now()
    logtimefile("\n\t\tTime for create observation circuits and prod circuit: "+ str((time25- time2).seconds))
    cmd = ["cp", f"{outFolder}/prod.v", f"{outFolder}/prod.old"]
    ...
    # 8. run the BMC to get a ctx or pass
    log("Verification")
    verifMode="yosys-smt"
    targetName = CONF.prodCircuitTemplate.replace(".v", "")
    avrPath = CONF.avrPath
    if verifMode == "yosys-smt":
        ...
        # cmd = [CONF.yosysBMCPath, "-s", "z3"]
        log("Bounded model checking")
        if CONF.yosysBMCSolver != "z3":
            cmd = [CONF.yosysBMCPath, "-s", CONF.yosysBMCSolver]
        else:
            cmd = [CONF.yosysBMCPath, "-s", CONF.yosysBMCSolver, "-S", "parallel.enable=true"]
        
        if filtertype == "delayedcheck":
            cmd += ["-t", str(int(CONF.BMCBound)+1)]
        ...
        if "Status: FAILED" in output:
            log("Verification FAILED")
            return "FAIL","{}/{}_tb.v".format(outFolder, targetName), trg_obs_dict, src_cand_obs_dict

        elif "Status: PASSED" in output:
            log("Verification PASSED")
            return "PASS", None, trg_obs_dict, src_cand_obs_dict
        else:
            print("Unknown verification result")
            exit(1)