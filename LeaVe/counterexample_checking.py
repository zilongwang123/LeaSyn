        ...
        if diffcycle:
            ...
            for inv in invs:
                ...
                if (tbname[-1] == "0" or tbname[-1] == "x") and len(tbname) == 2:
                    ...
                    if  name == "HLT" or name == "Retire" or name == "decode_branch_taken" or name == "retire_branch_taken" or name == "retire_FLUSH":
                        # if name == "Retire" :
                        logtimefile("\nFailed to prove " + name)
                        exit(1)
...
def runCounterexample(counterexample, trgObservations, cstrtype, filtertype):
    ...
    # exit(1)
    run_process(["cp", "{}/prod_tb.v".format(outFolder), "{}/prod_tb.norename".format(outFolder)])
