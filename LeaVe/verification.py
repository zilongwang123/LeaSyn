    ...
    for obs in observations:
        ...
        for attr in obs.get("attrs"):
            ...
            if attr.get("init") is None:
                ...
                init = attr.get("init")
            argObs = { "var" : "{}_{}_arg{}".format(obsId, prefix, idx) , "expr" : attr.get("value") , "width" : width, "init": init}
...
def createModule(outFolder, module, obsDict, inputsDict, suffix):
    ...
    moduleSrc += "(\n "
    moduleSrc += ' ,\n '.join(["input {}".format(var) for var in inputVars] + ["output {}".format(var) for var in outputVars])
    moduleSrc += " \n);\n"
    for obsId in obsDict.keys():
        for obs in obsDict[obsId]:
...
def flatten(folder, filename, module):
    ...
    yosysScript += "flatten\n"
    yosysScript += "select {}\n".format(module)
    yosysScript += "write_verilog -selected -norename {}/{}.temp\n".format(folder, module)
    return yosysScript
...
def precomputing(srcObservations, trgObservations, stateInvariant, state, auxVars, metaVars, filtertype):
    module = CONF.module
    ...
    ## 4. inline initial state variables
    ...
    yosysScript += "write_verilog {}/{}_nonrenamed.temp\n".format(outFolder_base, targetName)
...
def verify(trgObservations, cstrtype, filtertype):
    ...
    ################ 1. replace the right trg_equiv in the prod_renamed.temp
    # todo: generate non renamed version and use it for verification
    run_process(["cp", "{}/prod_nonrenamed.temp".format(outFolder), "{}/prod.v".format(outFolder)])
